...
#  validates_inclusion_of :content_type, :in => attachment_options[:content_type], :message => "is not allowed", :allow_nil => true
  ...
  #Named scopes
  named_scope :recent, :order => "photos.created_at DESC"
  named_scope :new_this_week, :order => "photos.created_at DESC", :conditions => ["photos.created_at > ?", 7.days.ago.to_s(:db)]
  ...
  def description_for_rss
    "<a href='#{self.link_for_rss}' title='#{self.name}'><img src='#{self.avatar.url(:large)}' alt='#{self.name}' /><br />#{self.description}</a>"
  end
